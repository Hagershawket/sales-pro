...
class SaleController extends Controller
{
    ...
    {      
        ...
        if($role->hasPermissionTo('sales-index')) {
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
                $lims_sale_all = Sale::orderBy('id', 'desc')->where('user_id', Auth::id())->where('vendor_id', $vendor_id)->get();
            else
                $lims_sale_all = Sale::orderBy('id', 'desc')->where('vendor_id', $vendor_id)->get();

            $lims_gift_card_list = GiftCard::where("is_active", true)->where('vendor_id', $vendor_id)->get();
            $lims_pos_setting_data = PosSetting::latest()->first();
            $lims_account_list = Account::where('is_active', true)->where('vendor_id', $vendor_id)->get();
    ...
    public function saleData(Request $request)
    {
        ...
        if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
            $totalData = Sale::where('user_id', Auth::id())->where('vendor_id', $vendor_id)->count();
        else
            $totalData = Sale::where('vendor_id', $vendor_id)->count();
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            $lims_customer_list = Customer::where('is_active', true)->where('vendor_id', Auth::user()->v_id)->get();
            if(Auth::user()->role_id > 2) {
                ...
                $lims_biller_list = Biller::where('is_active', true)->where('vendor_id', Auth::user()->v_id)->get();
            }

            $lims_tax_list = Tax::where('is_active', true)->where('vendor_id', Auth::user()->v_id)->get();
            $lims_pos_setting_data = PosSetting::latest()->first();
    ...
    public function create2()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            return view('sale.create2',compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
        ...
        foreach ($product_id as $i => $id) {
            ...
            if($sale_unit[$i] != 'n/a') {
                ...
                if($lims_product_data->is_variant) {
                    ...
                    $lims_product_variant_data = ProductVariant::select('id', 'variant_id', 'qty')->FindExactProductWithCode($id, $product_id[$i])->first();
            ...
            $product_sale['variant_id'] = $data['Product-Varient'];
    ...
    public function getProduct($id)
    {
        ...
        foreach ($lims_product_warehouse_data as $product_warehouse) 
        {
            ...
            $product_img[] = $lims_product_data->image;
        }
        ...
        foreach ($lims_product_with_variant_warehouse_data as $product_warehouse) 
        {
            ...
            $product_list[] = $lims_product_data->product_list;
            $qty_list[] = $lims_product_data->qty_list;
            // foreach($lims_product_variant_data as $item){
            //     // $product_img[] = Product::select("image")->where('id', $item['product_id']);
            //     // dd($item['product_id']);
            //  }
        }
        ...
        //return $product_img;
        $product_data = [$product_code, $product_name, $product_qty, $product_type, $product_id, $product_list, $qty_list, $product_price , $product_img];
        return $product_data;
    }
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            if(empty($all_permission))
                $all_permission[] = 'dummy text';
            $lims_customer_list = Customer::where('is_active', true)->where('vendor_id', $vendor_id)->get();
            $lims_customer_group_all = CustomerGroup::where('is_active', true)->where('vendor_id', $vendor_id)->get();
            $lims_warehouse_list = Warehouse::where('is_active', true)->where('vendor_id', $vendor_id)->get();
            $lims_biller_list = Biller::where('is_active', true)->where('vendor_id', $vendor_id)->get();
            $lims_tax_list = Tax::where('is_active', true)->where('vendor_id', $vendor_id)->get();
            $lims_product_list = Product::select('id', 'name', 'code', 'image')->where('vendor_id', $vendor_id)->ActiveFeatured()->whereNull('is_variant')->get();
            foreach ($lims_product_list as $key => $product) {
                $images = explode(",", $product->image);
                $product->base_image = $images[0];
            }
            $lims_product_list_with_variant = Product::select('id', 'name', 'code', 'image')->where('vendor_id', $vendor_id)->ActiveFeatured()->whereNotNull('is_variant')->get();
            ...
            $lims_pos_setting_data = PosSetting::latest()->first();
            $lims_brand_list = Brand::where('is_active',true)->where('vendor_id', $vendor_id)->get();
            $lims_category_list = Category::where('is_active',true)->where('vendor_id', $vendor_id)->get();
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                ...
                $recent_sale = Sale::where('sale_status', 1)->where('vendor_id', $vendor_id)->orderBy('id', 'desc')->take(10)->get();
                $recent_draft = Sale::where('sale_status', 3)->where('vendor_id', $vendor_id)->orderBy('id', 'desc')->take(10)->get();
            }
            $lims_coupon_list = Coupon::where('is_active',true)->where('vendor_id', $vendor_id)->get();
            $flag = 0;
