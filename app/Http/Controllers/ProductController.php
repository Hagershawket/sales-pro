...
class ProductController extends Controller
{
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('products-add')){
            $lims_product_list = Product::where([ ['is_active', true], ['type', 'standard'] ])->where('vendor_id', $vendor_id)->get();
            ...
            $lims_unit_list = Unit::where('is_active', true)->where('vendor_id', $vendor_id)->get();
    ...
    public function createExistProduct(Request $request)
    {
        $product_code = $request->product_code;
        $default_product_data = DefaultProduct::where('code', $product_code)->first();
        $vendor_id = Auth::user()->v_id;
        $role = Role::firstOrCreate(['id' => Auth::user()->role_id]);
        if ($role->hasPermissionTo('products-add')){
            $lims_product_list = Product::where([ ['is_active', true], ['type', 'standard'] ])->where('vendor_id', $vendor_id)->get();
            ...
            $lims_tax_list = Tax::where('is_active', true)->where('vendor_id', $vendor_id)->get();
            $lims_warehouse_list = Warehouse::where('is_active', true)->where('vendor_id', $vendor_id)->get();
            return view('product.create2',compact('default_product_data', 'lims_product_list', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_warehouse_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
        ...
        $this->validate($request, [
            'code' => [
                'max:255',
                    ...
                    return $query->where('is_active', 1)->where('vendor_id', Auth::user()->v_id);
                }),
            ],
            'name' => [
                'max:255',
    ...
    public function storeExistProduct(Request $request)
    {
        $this->validate($request, [
            'code' => [
                'max:255',
    ...
    {   
        ...
        if ($role->hasPermissionTo('products-edit')) {
            ...
            $lims_unit_list = Unit::where('is_active', true)->where('vendor_id',$vendor_id)->get();
            $lims_tax_list = Tax::where('is_active', true)->where('vendor_id',$vendor_id)->get();
            $lims_product_data = Product::where('id', $id)->where('vendor_id',$vendor_id)->first();
    ...
    public function search(Request $request)
    {
        ...
        $lims_product_data = Product::where('code', $product_code[0])->where('vendor_id',$vendor_id)->first();
    ...
    {
        $vendor_id = Auth::user()->v_id;
        $unit = Unit::where('vendor_id',$vendor_id)->where("base_unit", $id)->orWhere('id', $id)->pluck('unit_name','id');
    ...
    {
        $vendor_id = Auth::user()->v_id;
        $lims_product_list = Product::where('is_active', true)->where('vendor_id',$vendor_id)->get();
