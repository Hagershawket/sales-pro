...
class AttendanceController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('attendance')) {
            $lims_employee_list = Employee::where('is_active', true)->where('vendor_id', $vendor_id)->get();
            $lims_hrm_setting_data = HrmSetting::latest()->first();
            $general_setting = DB::table('general_settings')->latest()->first();
            if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own')
                $lims_attendance_all = Attendance::orderBy('id', 'desc')->where('user_id', Auth::id())->where('vendor_id', $vendor_id)->get();
            else
                $lims_attendance_all = Attendance::orderBy('id', 'desc')->where('vendor_id', $vendor_id)->get();
    ...
    public function store(Request $request)
    {
        ...
        foreach ($employee_id as $id) {
            ...
            $lims_attendance_data = Attendance::whereDate('date', $data['date'])->where('employee_id', $id)->where('vendor_id', $vendor_id)->first();
