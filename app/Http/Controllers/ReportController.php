...
class ReportController extends Controller
{
    public function productQuantityAlert()
    {
        ...
        if($role->hasPermissionTo('product-qty-alert')){
            $lims_product_data = Product::select('name','code', 'image', 'qty', 'alert_quantity')->where('is_active', true)->where('vendor_id', $vendor_id)->whereColumn('alert_quantity', '>', 'qty')->get();
        ...
        if($role->hasPermissionTo('warehouse-stock-report')){
            ...
            $total_item = DB::table('product_warehouse')
                        ...
                        ->where([
                            ...
                            ['products.vendor_id', Auth::user()->v_id],
                            ['product_warehouse.qty', '>' , 0]
                        ])->count();

            $total_qty = Product::where('is_active', true)->where('vendor_id', Auth::user()->v_id)->sum('qty');
            $total_price = DB::table('products')->where('is_active', true)->where('vendor_id', Auth::user()->v_id)->sum(DB::raw('price * qty'));
            $total_cost = DB::table('products')->where('is_active', true)->where('vendor_id', Auth::user()->v_id)->sum(DB::raw('cost * qty'));
            $lims_warehouse_list = Warehouse::where('is_active', true)->where('vendor_id', Auth::user()->v_id)->get();
            $warehouse_id = 0;
    ...
    public function dailySale($year, $month)
    {
        ...
        if($role->hasPermissionTo('daily-sale')){
            ...
            while($start <= $number_of_day)
            {
                ...
                $sale_data = Sale::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', $date)->selectRaw(implode(',', $query1))->get();
    ...
    public function dailySaleByWarehouse(Request $request,$year,$month)
    {
        ...
        while($start <= $number_of_day)
        {
            ...
            $sale_data = Sale::where('warehouse_id', $data['warehouse_id'])->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', $date)->selectRaw(implode(',', $query1))->get();
            $total_discount[$start] = $sale_data[0]->total_discount;
            $order_discount[$start] = $sale_data[0]->order_discount;
            $total_tax[$start] = $sale_data[0]->total_tax;
            $order_tax[$start] = $sale_data[0]->order_tax;
    ...
    public function dailyPurchase($year, $month)
    {
        ...
        if($role->hasPermissionTo('daily-purchase')){
            ...
            while($start <= $number_of_day)
            {
                ...
                $purchase_data = Purchase::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', $date)->selectRaw(implode(',', $query1))->get();
    ...
    {        
        ...
        while($start <= $number_of_day)
        {
            ...
            $purchase_data = Purchase::where('vendor_id', Auth::user()->v_id)->where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', $date)->selectRaw(implode(',', $query1))->get();
            $total_discount[$start] = $purchase_data[0]->total_discount;
            $order_discount[$start] = $purchase_data[0]->order_discount;
            $total_tax[$start] = $purchase_data[0]->total_tax;
            $order_tax[$start] = $purchase_data[0]->order_tax;
    ...
    public function monthlySale($year)
    {
        ...
        if($role->hasPermissionTo('monthly-sale')){
            ...
            while($start <= $end)
            {
                ...
                $temp_total_discount = Sale::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('total_discount');
                $total_discount[] = number_format((float)$temp_total_discount, 2, '.', '');

                $temp_order_discount = Sale::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('order_discount');
                $order_discount[] = number_format((float)$temp_order_discount, 2, '.', '');

                $temp_total_tax = Sale::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('total_tax');
                $total_tax[] = number_format((float)$temp_total_tax, 2, '.', '');

                $temp_order_tax = Sale::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('order_tax');
                $order_tax[] = number_format((float)$temp_order_tax, 2, '.', '');

                $temp_shipping_cost = Sale::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('shipping_cost');
                $shipping_cost[] = number_format((float)$temp_shipping_cost, 2, '.', '');

                $temp_total = Sale::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('grand_total');
    ...
    public function monthlySaleByWarehouse(Request $request, $year)
    {
        ...
        while($start <= $end)
        {
            ...
            $temp_total_discount = Sale::where('vendor_id', Auth::user()->v_id)->where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('total_discount');
            $total_discount[] = number_format((float)$temp_total_discount, 2, '.', '');

            $temp_order_discount = Sale::where('vendor_id', Auth::user()->v_id)->where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('order_discount');
            $order_discount[] = number_format((float)$temp_order_discount, 2, '.', '');

            $temp_total_tax = Sale::where('vendor_id', Auth::user()->v_id)->where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('total_tax');
            $total_tax[] = number_format((float)$temp_total_tax, 2, '.', '');

            $temp_order_tax = Sale::where('vendor_id', Auth::user()->v_id)->where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('order_tax');
            $order_tax[] = number_format((float)$temp_order_tax, 2, '.', '');

            $temp_shipping_cost = Sale::where('vendor_id', Auth::user()->v_id)->where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('shipping_cost');
            $shipping_cost[] = number_format((float)$temp_shipping_cost, 2, '.', '');

            $temp_total = Sale::where('vendor_id', Auth::user()->v_id)->where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('grand_total');
            $total[] = number_format((float)$temp_total, 2, '.', '');
            $start = strtotime("+1 month", $start);
        }
        $lims_warehouse_list = Warehouse::where('is_active',true)->where('vendor_id', Auth::user()->v_id)->get();
    ...
    public function monthlyPurchase($year)
    {
        ...
        if($role->hasPermissionTo('monthly-purchase')){
            ...
            while($start <= $end)
            {
                ...
                $purchase_data = Purchase::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query1))->get();
    ...
    public function monthlyPurchaseByWarehouse(Request $request, $year)
    {
        ...
        while($start <= $end)
        {
            ...
            $purchase_data = Purchase::where('vendor_id', Auth::user()->v_id)->where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query1))->get();
    ...
    public function bestSeller()
    {
        ...
        if($role->hasPermissionTo('best-seller')){
            ...
            while($start <= $end)
            {
                ...
                $best_selling_qty = Product_Sale::select(DB::raw('product_id, sum(qty) as sold_qty'))->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->groupBy('product_id')->orderBy('sold_qty', 'desc')->take(1)->get();
    ...
    public function bestSellerByWarehouse(Request $request)
    {
        ...
        while($start <= $end)
        {
            ...
            $best_selling_qty = DB::table('sales')
                                ...
                                ->where('sales.vendor_id', Auth::user()->v_id)
    ...
    public function profitLoss(Request $request)
    {
        ...
        $purchase = Purchase::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query1))->get();
        $total_purchase = Purchase::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->count();
        $sale = Sale::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query1))->get();
        $total_sale = Sale::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->count();
        $return = Returns::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query2))->get();
        $total_return = Returns::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->count();
        $purchase_return = ReturnPurchase::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query2))->get();
        $total_purchase_return = ReturnPurchase::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->count();
        $expense = Expense::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('amount');
        $total_expense = Expense::where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->count();
        ...
        $lims_warehouse_all = Warehouse::where('is_active',true)->where('vendor_id', Auth::user()->v_id)->get();
        $warehouse_name = [];
        foreach ($lims_warehouse_all as $warehouse) {
            $warehouse_name[] = $warehouse->name;
            $warehouse_sale[] = Sale::where('warehouse_id', $warehouse->id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query2))->get();
            $warehouse_purchase[] = Purchase::where('warehouse_id', $warehouse->id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query2))->get();
            $warehouse_return[] = Returns::where('warehouse_id', $warehouse->id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query2))->get();
            $warehouse_purchase_return[] = ReturnPurchase::where('warehouse_id', $warehouse->id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query2))->get();
            $warehouse_expense[] = Expense::where('warehouse_id', $warehouse->id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('amount');
        }
    ...
    public function productReport(Request $request)
    {
        ...
        $product_name = [];
        $product_qty = [];
        $lims_product_all = Product::select('id', 'name', 'qty', 'is_variant')->where('is_active', true)->where('vendor_id', Auth::user()->v_id)->get();
        foreach ($lims_product_all as $product) {
            $lims_product_purchase_data = null;
            $variant_id_all = [];
            if($warehouse_id == 0) {
                if($product->is_variant)
                    $variant_id_all = ProductPurchase::distinct('variant_id')->where('product_id', $product->id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->pluck('variant_id');
                else
                    $lims_product_purchase_data = ProductPurchase::where('product_id', $product->id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->first();
            }
            else {
                if($product->is_variant)
                    $variant_id_all = DB::table('purchases')
                        ...
                        ->where([
                            ...
                            ['purchases.vendor_id', Auth::user()->v_id]
            ...
            elseif(count($variant_id_all)) {
                ...
                if($warehouse_id == 0) {
                    if($product->is_variant){
                        $variant_id_all = Product_Sale::distinct('variant_id')->where('product_id', $product->id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->pluck('variant_id');
                        //return $lims_product_sale_data;
                    }
                    else
                        $lims_product_sale_data = Product_Sale::where('product_id', $product->id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->first();
                }
                else {
                    if($product->is_variant)
                        $variant_id_all = DB::table('sales')
                            ...
                            ->where([
                                ...
                                ['sales.vendor_id', Auth::user()->v_id]
                            ])->whereDate('sales.created_at','>=', $start_date)
                              ->whereDate('sales.created_at','<=', $end_date)
                              ->pluck('variant_id');
                    else
    ...
    public function warehouseReport(Request $request)
    {
        ...
        $lims_purchase_data = Purchase::where('vendor_id', Auth::user()->v_id)->where('warehouse_id', $warehouse_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
        $lims_sale_data = Sale::with('customer')->where('vendor_id', Auth::user()->v_id)->where('warehouse_id', $warehouse_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
        ...
        $lims_return_data = Returns::with('customer', 'biller')->where('vendor_id', Auth::user()->v_id)->where('warehouse_id', $warehouse_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
        $lims_expense_data = Expense::with('expenseCategory')->where('vendor_id', Auth::user()->v_id)->where('warehouse_id', $warehouse_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
        ...
        foreach ($lims_purchase_data as $key => $purchase) {
            $lims_product_purchase_data[$key] = ProductPurchase::where('purchase_id', $purchase->id)->where('vendor_id', Auth::user()->v_id)->get();
        }
        foreach ($lims_sale_data as $key => $sale) {
            $lims_product_sale_data[$key] = Product_Sale::where('sale_id', $sale->id)->where('vendor_id', Auth::user()->v_id)->get();
        }
    ...
    public function userReport(Request $request)
    {
        ...
        $lims_sale_data = Sale::with('customer', 'warehouse')->where('user_id', $user_id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
        $lims_purchase_data = Purchase::with('warehouse')->where('user_id', $user_id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();        
        ...
        $lims_transfer_data = Transfer::with('fromWarehouse', 'toWarehouse')->where('user_id', $user_id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();        
        $lims_payment_data = DB::table('payments')
                           ...
                           ->get();
        $lims_expense_data = Expense::with('warehouse', 'expenseCategory')->where('user_id', $user_id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
        ...
        foreach ($lims_transfer_data as $key => $transfer) {
            $lims_product_transfer_data[$key] = ProductTransfer::where('transfer_id', $transfer->id)->where('vendor_id', Auth::user()->v_id)->get();
        }

        $lims_user_list = User::where('is_active', true)->where('v_id', Auth::user()->v_id)->get();
    ...
    public function customerReport(Request $request)
    {
        ...
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        $lims_sale_data = Sale::with('warehouse')->where('customer_id', $customer_id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
        ...
        $lims_return_data = Returns::with('warehouse', 'biller')->where('customer_id', $customer_id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
        $lims_payment_data = DB::table('payments')
        ...
        $lims_customer_list = Customer::where('is_active', true)->where('vendor_id', Auth::user()->v_id)->orwhere('id','=', 0)->get();
    ...
    public function supplierReport(Request $request)
    {
        ...
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        $lims_purchase_data = Purchase::with('warehouse')->where('supplier_id', $supplier_id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
        ...
        $lims_return_data = ReturnPurchase::with('warehouse')->where('supplier_id', $supplier_id)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
        $lims_payment_data = DB::table('payments')
        ...
        $lims_supplier_list = Supplier::where('is_active', true)->where('vendor_id', Auth::user()->v_id)->get();
    ...
    public function dueReportByDate(Request $request)
    {
        ...
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        $lims_sale_data = Sale::where('payment_status', '!=', 4)->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->get();
