    ...
    public function index()
    {
        ...
        if($role->hasPermissionTo('returns-index')){
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
                $lims_return_all = Returns::with('biller', 'customer', 'warehouse', 'user')->orderBy('id', 'desc')->orderBy('id', 'desc')->where('vendor_id',$vendor_id)->where('user_id', Auth::id())->get();
            else
                $lims_return_all = Returns::with('biller', 'customer', 'warehouse', 'user')->orderBy('id', 'desc')->where('vendor_id',$vendor_id)->get();
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('returns-add')){
            ...
            if(Auth::user()->role_id <= 2) {
                ...
                $lims_warehouse_list = Warehouse::where('id',Auth::user()->warehouse_id)->where('vendor_id',$vendor_id)->get();
                $lims_biller_list = Biller::where('id', Auth::user()->biller_id)->where('vendor_id',$vendor_id)->get();
            }
