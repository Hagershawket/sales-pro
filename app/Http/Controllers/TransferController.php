    ...
    public function index()
    {
        ...
        if($role->hasPermissionTo('transfers-index')){
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
                $lims_transfer_all = Transfer::with('fromWarehouse', 'toWarehouse', 'user')->orderBy('id', 'desc')->where('user_id', Auth::id())->where('vendor_id',$vendor_id)->get();
            else
                $lims_transfer_all = Transfer::with('fromWarehouse', 'toWarehouse', 'user')->orderBy('id', 'desc')->where('vendor_id',$vendor_id)->get();
