...
class UnitController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('unit')) {
            $lims_unit_all = Unit::where('is_active', true)->where('vendor_id',$vendor_id)->get();
    ...
    public function store(Request $request)
    {
            ...
            'unit_code' => [
                'max:255',
                    Rule::unique('units')->where(function ($query) {
                        $vendor_id = Auth::user()->v_id;
                        return $query->where('is_active', 1)->where('vendor_id',$vendor_id);
                }),
            ],
        ...
        $input['vendor_id'] = Auth::user()->v_id;
        Unit::create($input);
