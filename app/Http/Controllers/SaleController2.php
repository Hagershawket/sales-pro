...
class SaleController2 extends Controller
{
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            $lims_customer_list = Customer::where('is_active', true)->where('vendor_id',$vendor_id)->orwhere('id','=',0)->get();
    ...
    {
        // return $id;
        //return [ ["Hello" , "World"] , ["Welcome" , "Welcome2"]];
        ...
        foreach ($lims_product_warehouse_data as $product_warehouse) 
        {
            ...
            $tax_method[]   = $product_warehouse->tax_method;
            $qunted         = Product_Sale::where('product_id', '=' , $lims_product_data->id)->sum('qty');
            $saledProduct   = ProductPurchase::where('product_id', '=' , $lims_product_data->id)->sum('qty');
            //$expected[] = $product_warehouse->qty - $qunted;
            $expected[] = $saledProduct - $qunted;


            //$differnce = ($saledProduct - $qunted) -
        }
        ...
        foreach ($lims_product_with_variant_warehouse_data as $product_warehouse) 
        {
            ...
            //     // $tax_method[] = Product::select("tax_method")->where('id', $item['product_id']);
            // }
        ...
        //     $qunted         = Product_Sale::where('product_id', '=' , $product->id)->sum('qty');
        //     $saledProduct   = ProductPurchase::where('product_id', '=' , $product->id)->sum('qty');
        //     $expected[] = $saledProduct - $qunted;
             $qunted         = Product_Sale::where('product_id', '=' , $product->id)->sum('qty');
             $expected[] = $product->qty - $qunted;
            
        }
        // dd("Hello World");
        $product_data = [$product_code, $product_name, $product_qty , $product_price, $product_id, $product_list, $qty_list, $product_price , $expected];
        return $product_data;
    }
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            $lims_biller_list = Biller::where('is_active', true)->where('vendor_id',$vendor_id)->get();
            $lims_tax_list = Tax::where('is_active', true)->where('vendor_id',$vendor_id)->get();
            $lims_product_list = Product::select('id', 'name', 'code', 'image')->where('v_id',$vendor_id)->ActiveFeatured()->whereNull('is_variant')->get();
            foreach ($lims_product_list as $key => $product) {
                $images = explode(",", $product->image);
                $product->base_image = $images[0];
            }
            $lims_product_list_with_variant = Product::select('id', 'name', 'code', 'image')->where('v_id',$vendor_id)->ActiveFeatured()->whereNotNull('is_variant')->get();
            ...
            $lims_pos_setting_data = PosSetting::latest()->first();
            $lims_brand_list = Brand::where('is_active',true)->where('v_id',$vendor_id)->get();
            $lims_category_list = Category::where('is_active',true)->where('v_id',$vendor_id)->get();
