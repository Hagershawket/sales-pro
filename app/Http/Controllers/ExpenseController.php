    ...
    public function index()
    {
        ...
        if($role->hasPermissionTo('expenses-index')){
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
                $lims_expense_all = Expense::orderBy('id', 'desc')->where('user_id', Auth::id())->where('vendor_id', $vendor_id)->get();
            else
                $lims_expense_all = Expense::orderBy('id', 'desc')->where('vendor_id', $vendor_id)->get();
