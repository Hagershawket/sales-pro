...
class PurchaseController extends Controller
{
    ...
    public function purchaseData(Request $request)
    {
        ...
        if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
            $totalData = Purchase::where('user_id', Auth::id())->where('vendor_id', $vendor_id)->count();
        else
            $totalData = Purchase::where('vendor_id', $vendor_id)->count();
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('purchases-add')){
            //return "Hello";
            $lims_supplier_list = Supplier::where('is_active', true)->where('vendor_id',$vendor_id)->get();
    ...
    {
        $vendor_id = Auth::user()->v_id;
        return Product::ActiveStandard()->select('id', 'name', 'code')->where('vendor_id',$vendor_id)
    ...
    {
        $vendor_id = Auth::user()->v_id;
        return Product::join('product_variants', 'products.id', 'product_variants.product_id')->where('vendor_id',$vendor_id)
                ->ActiveStandard()
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        if(!$lims_product_data) {
            $lims_product_data = Product::join('product_variants', 'products.id', 'product_variants.product_id')
                ...
                ->where('vendor_id',$vendor_id)
                ->orwhere('product_variants.item_code', $product_code[0])
                ->first();
        }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($product_id as $i => $id) {
            ...
            if ($lims_product_warehouse_data) {
                ...
                $lims_product_warehouse_data->vendor_id = Auth::user()->v_id;
                if($lims_product_data->is_variant)
            ...
            $product_purchase['vendor_id'] = Auth::user()->v_id;
