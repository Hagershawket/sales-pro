...
class AccountsController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('account-index')){
            $lims_account_all = Account::where('is_active', true)->where('vendor_id', $vendor_id)->get();
    ...
    {
        $lims_account_data = Account::where('is_default', true)->where('vendor_id', Auth::user()->v_id)->first();
    ...
    public function balanceSheet()
    {
        ...
        if($role->hasPermissionTo('balance-sheet')){
            $lims_account_list = Account::where('is_active', true)->where('vendor_id', Auth::user()->v_id)->get();
            $debit = [];
            $credit = [];
            foreach ($lims_account_list as $account) {
                ...
                $returns = DB::table('returns')->where('account_id', $account->id)->where('vendor_id', Auth::user()->v_id)->sum('grand_total');
                $return_purchase = DB::table('return_purchases')->where('account_id', $account->id)->where('vendor_id', Auth::user()->v_id)->sum('grand_total');
                $expenses = DB::table('expenses')->where('account_id', $account->id)->where('vendor_id', Auth::user()->v_id)->sum('amount');
                $payrolls = DB::table('payrolls')->where('account_id', $account->id)->where('vendor_id', Auth::user()->v_id)->sum('amount');
                $sent_money_via_transfer = MoneyTransfer::where('from_account_id', $account->id)->where('vendor_id', Auth::user()->v_id)->sum('amount');
                $recieved_money_via_transfer = MoneyTransfer::where('to_account_id', $account->id)->where('vendor_id', Auth::user()->v_id)->sum('amount');
    ...
    public function accountStatement(Request $request)
    {
        ...
        if($data['type'] == '0' || $data['type'] == '2') {
            ...
            $recieved_money_transfer_list = MoneyTransfer::where('to_account_id', $data['account_id'])->where('vendor_id', Auth::user()->v_id)->get();
        }
        if($data['type'] == '0' || $data['type'] == '1'){
            ...
            $expense_list = Expense::where('account_id', $data['account_id'])->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $data['start_date'])->whereDate('created_at', '<=' , $data['end_date'])->get();

            $return_list = Returns::where('account_id', $data['account_id'])->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $data['start_date'])->whereDate('created_at', '<=' , $data['end_date'])->get();

            $purchase_return_list = ReturnPurchase::where('account_id', $data['account_id'])->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $data['start_date'])->whereDate('created_at', '<=' , $data['end_date'])->get();

            $payroll_list = Payroll::where('account_id', $data['account_id'])->where('vendor_id', Auth::user()->v_id)->whereDate('created_at', '>=' , $data['start_date'])->whereDate('created_at', '<=' , $data['end_date'])->get();

            $sent_money_transfer_list = MoneyTransfer::where('from_account_id', $data['account_id'])->where('vendor_id', Auth::user()->v_id)->get();
        }
        $balance = 0;
