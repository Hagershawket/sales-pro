    ...
    public function index()
    {
        ...
        if($role->hasPermissionTo('purchase-return-index')){
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
                $lims_return_all = ReturnPurchase::with('supplier', 'warehouse', 'user')->orderBy('id', 'desc')->where('user_id', Auth::id())->where('vendor_id',$vendor_id)->get();
            else
                $lims_return_all = ReturnPurchase::with('supplier', 'warehouse', 'user')->orderBy('id', 'desc')->where('vendor_id',$vendor_id)->get();
