...
class ProductController extends Controller
{
    ...
    public function index()
    {
        ...
        $permission_active = $this->checkPermission($role , 'products-index');
        if($permission_active){            
            ...
            return view('admins.product.index', compact('all_permission'));
        }
        else
    ...
    public function productData(Request $request)
    {
        ...
        $totalData = DefaultProduct::where('is_active', true)->count();
        ...
        $order = 'default_products.'.$columns[$request->input('order.0.column')];
        $dir = $request->input('order.0.dir');
        if(empty($request->input('search.value'))){
            $products = DefaultProduct::with('category', 'brand', 'unit')->offset($start)
        ...
        else
        {
            ...
            $products =  DefaultProduct::select('products.*')
                        ->with('category', 'brand', 'unit')
                        ->join('categories', 'default_products.category_id', '=', 'categories.id')
                        ->leftjoin('brands', 'default_products.brand_id', '=', 'brands.id')
                        ->where([
                            ['default_products.name', 'LIKE', "%{$search}%"],
                            ['default_products.is_active', true]
                        ])
                        ->orWhere([
                            ['default_products.code', 'LIKE', "%{$search}%"],
                            ['default_products.is_active', true]
                        ])
                        ->orWhere([
            ...
            foreach ($products as $key=>$product)
            {
                ...
                if(in_array("products-edit", $request['all_permission']))
                    $nestedData['options'] .= '<li>
                            <a href="'.route('admin.products.edit', $product->id).'" class="btn btn-link"><i class="fa fa-edit"></i> '.trans('file.edit').'</a>
                        </li>';
                if(in_array("products-delete", $request['all_permission']))
                    $nestedData['options'] .= \Form::open(["route" => ["admin.products.destroy", $product->id], "method" => "DELETE"] ).'
                            <li>
    ...
    {
        $role = Role::firstOrCreate(['id' => Auth::guard('admin')->user()->role_id]);
        $permission_active = $this->checkPermission($role , 'products-add');
        if ($permission_active){
            ...
            return view('admins.product.create',compact('lims_product_list', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_warehouse_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            'code' => [
                'max:255',
                    Rule::unique('default_products')->where(function ($query) {
        ...
        $lims_product_data = DefaultProduct::create($data);
    ...
    {
        $role = Role::firstOrCreate(['id' => Auth::guard('admin')->user()->role_id]);
        $permission_active = $this->checkPermission($role , 'products-edit');
        if ($permission_active) {
            $lims_product_list = DefaultProduct::where([ ['is_active', true], ['type', 'standard'] ])->get();
            $lims_brand_list = Brand::where('is_active', true)->get();
            $lims_category_list = Category::where('is_active', true)->get();
            $lims_unit_list = Unit::where('is_active', true)->get();
            $lims_tax_list = Tax::where('is_active', true)->get();
            $lims_product_data = DefaultProduct::where('id', $id)->first();
            //$lims_product_variant_data=DB::table('product_variants')->where('product_id', $lims_product_data->id)->orderBy('position')->get();
           // $lims_product_variant_data = $lims_product_data->variant()->orderBy('position')->get();
            //$lims_warehouse_list = Warehouse::where('is_active', true)->get();
            return view('admins.product.edit',compact('lims_product_list', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function updateProduct(Request $request)
    {
        if(!env('USER_VERIFIED')) {
            ...
            $this->validate($request, [
                'name' => [
                    'max:255',
                    Rule::unique('default_products')->ignore($request->input('id'))->where(function ($query) {
                        return $query->where('is_active', 1);
                    }),
                ],
            ...
            $lims_product_data = DefaultProduct::findOrFail($request->input('id'));
    ...
    public function printBarcode()
    {
        ...
        return view('admins.product.print_barcode', compact('lims_product_list'));
    }
    ...
    {
        $lims_product_data = DefaultProduct::findOrFail($id);
        $lims_product_data->is_active = false;
        /*if($lims_product_data->image != 'zummXD2dvAtI.png') {
