...
class StockCountController extends Controller
{
    public function index()
    {
        ...
        if( $role->hasPermissionTo('stock_count') ) {
            ...
            $lims_brand_list = Brand::where('is_active', true)->where('vendor_id', Auth::user()->v_id)->get();
            $lims_category_list = Category::where('is_active', true)->where('vendor_id', Auth::user()->v_id)->get();
            $general_setting = DB::table('general_settings')->latest()->first();
            if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own')
                $lims_stock_count_all = StockCount::orderBy('id', 'desc')->where('user_id', Auth::id())->where('vendor_id', Auth::user()->v_id)->get();
            else
                $lims_stock_count_all = StockCount::orderBy('id', 'desc')->where('vendor_id', Auth::user()->v_id)->get();
